                ...
                <form action="{{ route('departments.update', $department) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="description"
                                  id="description"
                                  rows="4"
                                  class="w-full border-gray-300 rounded-lg shadow-sm focus:border-emerald-500 focus:ring-emerald-500 @error('description') border-red-500 @enderror">{{ old('description', $department->description) }}</textarea>
                        @error('description')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                            ...
                            @foreach($lecturers as $lecturer)
                                <div class="flex items-center mb-2">
                                    <input type="checkbox"
                                           ...
                                           {{ in_array($lecturer->id, old('lecturer_ids', $department->users->pluck('id')->toArray())) ? 'checked' : '' }}>
